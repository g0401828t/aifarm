...
class LossEarlyStopper():
    ...
    def __init__(self, patience: int, verbose: bool, logger:logging.RootLogger=None):
        """초기화 
    ...
    def check_early_stopping(self, loss: float):
        ...
        if self.min_loss == np.Inf:
            ...
            self.patience_counter = 0
            msg = f"Early stopper, counter reset {self.patience_counter}, Validation loss decreased {self.min_loss} -> {loss}"
            print(msg)
            self.min_loss = loss
            # self.save_checkpoint(loss=loss, model=model)
